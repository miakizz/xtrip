...
while True:
	...
	if ring:
		jackTrip = subprocess.Popen(['/home/pi/jt/jacktrip', '-s', '-n', '1', '--bufstrategy', '-1',  '--udprt'], stdout=subprocess.PIPE)
	...
	if num in xband_numbers:
		...
		jackTrip = subprocess.Popen(['/home/pi/jt/jacktrip', '-c', str(peerAddr), '-n', '1', '--bufstrategy', '-1',  '--udprt'], stdout=subprocess.PIPE)
		while True:
			line = jackTrip.stdout.readline().decode("UTF-8")
			if not line:
				break
